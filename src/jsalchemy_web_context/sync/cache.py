...
from jsalchemy_web_context.sync import request
...
def setup_cache(default_ttl: int=None, redis_connection: str=None, separator: str=None):
    ...
    if redis_connection is not None:
        CACHE = redis.Redis.from_url(redis_connection) if isinstance(redis_connection, str) else redis_connection
...
def request_cache(*attr_paths, separator=None):
    """
    Decorator that caches a function's return value in a dictionary
...
def redis_cache(*attr_paths, ttl=CACHE_TTL, separator=None):
    """
    Decorator that caches a function's return value in a Redis hash.
    ...
    * Each cache entry is stored under a field that is built from
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            blob = CACHE.hget(cache_key, key)
            if blob is not None:
                # print(f"RC <- {cache_key} {key}")
                return pickle.loads(blob)

            # print(f"RC -> {cache_key} {key}")

            result = func(*args, **kwargs)
            CACHE.hsetex(cache_key, key, pickle.dumps(result), ex=ttl)
            return result
